...
/*   Updated: 2026/01/13 11:53:56 by miokrako         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	update_pwd_vars(t_env *env, char *old_pwd, char *new_dir)
{
	...
	// Chercher et mettre à jour OLDPWD
	current = env;
	found_oldpwd = 0;
	while (current)
	{
	...
	// Si OLDPWD n'existe pas, la créer (normalement ne devrait pas arriver)
	if (!found_oldpwd)
	{
		t_env *new_node = new_env_node_kv("OLDPWD", old_pwd);
		if (new_node)
			exp_add_env_node_back(&env, new_node);
	}

	// Mise à jour de PWD (code existant inchangé)
	if (new_dir && new_dir[0] == '/')
		new_pwd = get_new_pwd_absolute(new_dir);
	else if (new_dir)
		new_pwd = get_new_pwd_relative(old_pwd, new_dir);
